//! Helpers for loading application configuration from a TOML file and the
//! process environment.
//!
//! Configuration is layered from two sources: an optional TOML file and the
//! environment variables of the running process. The environment is applied on
//! top of the file, so a value set in the environment overrides the same value
//! in the file. Because the file source is optional, an application can be
//! configured entirely through environment variables.

use ext_config::{Config, ConfigError, Environment, File, FileFormat};
use serde::de::DeserializeOwned;

/// Loads configuration of type `T` from a TOML file and environment variables,
/// preferring the environment when both define the same value.
///
/// The TOML file at `config_path` is optional: if it is missing, configuration
/// is sourced entirely from the environment. Environment variables are matched
/// using `env_prefix` and override any value present in the file.
///
/// Nested fields are addressed by joining the prefix and the field path with a
/// double underscore. For example, with prefix `POOL` the field
/// `jds.listen_address` maps to the variable `POOL__JDS__LISTEN_ADDRESS`.
///
/// Fields named in `list_keys` are parsed as comma-separated lists, e.g.
/// `POOL__SUPPORTED_EXTENSIONS=1,2,3`. Keys must be given in their lowercase,
/// `snake_case` form (the same name used in the configuration struct).
...
mod tests {
    ...
    fn missing_required_field_errors() {
        // Neither file nor env supplies the required fields.
        let missing = env::temp_dir().join("loader-test-empty.toml");
        let result: Result<TestConfig, _> =
            load_config(missing.to_str().unwrap(), "EMPTY", list_keys());
